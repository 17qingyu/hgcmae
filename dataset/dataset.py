...
class HGMAEDataset(DGLDataset):
    ...
    def process(self):
        ...
        for src, dst in product(self.node_types, self.node_types):
            ...
            with open(file_name, "r") as f:
                edges = f.readlines()
            edges = [(int(u), int(v)) for u, v in (edge.split() for edge in edges)]
        ...
        positive_sample = generate_pos(
            g, self._k, self.target_type, self._device, self._pos_num
        )
        g.nodes[self.target_type].data["pos"] = positive_sample.cpu()

        for node_type in self.node_types:
            file_name = osp.join(self.raw_path, f"feature_{node_type}.npz")
            if os.path.exists(file_name):
                x = sp.load_npz(file_name)
                g.nodes[node_type].data["x"] = torch.from_numpy(x.todense()).to(
                    torch.float
                )
            else:
                g.nodes[node_type].data['x'] = self._generate_features(g, node_type)

        y = np.load(osp.join(self.raw_path, "labels.npy"))
        g.nodes[self.target_type].data["y"] = torch.from_numpy(y).to(torch.long)
    ...
    def _generate_features(self, g, node_type):
        num_node = self.num_nodes_dict[node_type]
        return torch.eye(num_node).to(torch.float)
...
class ACMDataset(HGMAEDataset):
    ...
    def __init__(
        ...
        super().__init__(
            "acm", k, pos_num, device, raw_dir, force_reload, verbose, transform
        )
...
class DBLPDataset(HGMAEDataset):
    ...
    def __init__(
        ...
        super().__init__(
            "dblp", k, pos_num, device, raw_dir, force_reload, verbose, transform
        )
...
class FreebaseDataset(HGMAEDataset):
    ...
    def __init__(
        ...
        super().__init__(
            "freebase", k, pos_num, device, raw_dir, force_reload, verbose, transform
        )
...
class AminerDataset(HGMAEDataset):
    ...
    def __init__(
        ...
        super().__init__(
            "aminer", k, pos_num, device, raw_dir, force_reload, verbose, transform
        )
