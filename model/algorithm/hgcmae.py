...
class HGCMAE(LightningModule):
    ...
    def forward(self, graph, mask_x_s, mask_x_t, target_type, mask_nodes):
        ...
        pos = graph.ndata["pos"][target_type]
    ...
    def training_step(self, batch, batch_idx):
        graph, target_type = batch
        
        mask_x_s, mask_x_t, mask_s, mask_t = self.mask(
            graph.ndata["x"], self.current_epoch, target_type)

        loss_target = self(graph, mask_x_s, mask_x_t, target_type, mask_s)
        loss_non = self(graph, mask_x_t, mask_x_s, target_type, mask_t)
